        ...
        if (decodedBatch === false){
            ...
            if((cmdID === 0x0a)|(cmdID === 0x8a)|(cmdID === 0x01)){
                ...
                if (  (clusterdID === 0x0402 ) & (attributID === 0x0000)) {
                    ...
                    tab.push({label:"Temperature" + decoded.zclheader.endpoint  ,value:(UintToInt(bytes[index]*256+bytes[index+1],2))/100, date:lDate}) ;
                };
                //humidity
...
const ignore_vars = [/*'application_id','device_id','downlink_key',*/'lora_bandwidth','lora_spreading_factor','data_rate_index','coding_rate','frequency','time','gateway_eui'];
//timestamp,fport,fcnt,payload,rssi,snr
