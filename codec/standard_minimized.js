    ...
    while(flag===0) {
        ...
        if ((csd[3] === "1") && (csd[4] === "0")) {
            ...
            if (csd[1] === "1") {
                qual = "exceed"
                if (i2===0){
                    countUp = decimalToBitString(bytes[i1 + 3 + ((length)*i)])
                    countUp = parseInt(countUp, 2)
                } else {
                    countUp= decimalToBitString(bytes[i1 + 4 + ((length)*i)])
                    countUp = parseInt(countUp, 2)
                }
            } else {
                qual = "fall"
                if(i2===0){
                    countDown = decimalToBitString(bytes[i1 + 3 + ((length)*i)])
                    countDown = parseInt(countDown, 2)
                } else {
                    countDown = decimalToBitString(bytes[i1 + 4 + ((length)*i)])
                    countDown = parseInt(countDown, 2)
                }
            }
            let temp = ""
    ...
    while(flag===0) {
        ...
        if ((csd[3] === "1") && (csd[4] === "0")) {
            ...
            if (csd[1] === "1") {
                qual = "exceed"
                if (i2===0){
                    countUp = decimalToBitString(bytes[i1 + 7 + ((length)*i)])
                    countUp = parseInt(countUp, 2)
                } else {
                    countUp= decimalToBitString(bytes[i1 + 8 + ((length)*i)])
                    countUp = parseInt(countUp, 2)
                }
            } else {
                qual = "fall"
                if(i2===0){
                    countDown = decimalToBitString(bytes[i1 + 7 + ((length)*i)])
                    countDown = parseInt(countDown, 2)
                } else {
                    countDown = decimalToBitString(bytes[i1 + 8 + ((length)*i)])
                    countDown = parseInt(countDown, 2)
                }
            }
            let temp = ""
            ...
            if (i2===0){
                if (function_type==="none"){
                    ...
                    gap = ((bytes[i1 + 4 + ((length)*i)] * 256 + bytes[i1 + 5 + ((length)*i)] + bytes[i1+6+((length)*i)]) / divider).toString()
                }
                else if (function_type==="int"){
    ...
    while(flag===0) {
        ...
        if ((csd[3] === "1") && (csd[4] === "0")) {
            ...
            if (i2===0){
                ...
                count = decimalToBitString(bytes[i1 + 5 + ((length)*i)])
                count = parseInt(count, 2)
                if (count>=128){
                    shift+=4
                    countUp=decimalToBitString(bytes[i1 + 6 + ((length)*i)]*256+bytes[i1 + 7 + ((length)*i)])
                    countUp=parseInt(countUp,2)
                    countDown=decimalToBitString(bytes[i1 + 8 + ((length)*i)]*256+bytes[i1 + 9 + ((length)*i)])
                    countDown=parseInt(countDown,2)
                ...
                count = decimalToBitString(bytes[i1 + 6 + ((length)*i)])
                count = parseInt(count, 2)
                if(count>=128){
                    shift+=4
                    countUp=decimalToBitString(bytes[i1 + 7 + ((length)*i)]*256+bytes[i1 + 8 + ((length)*i)])
                    countUp=parseInt(countUp,4)
                    countDown=decimalToBitString(bytes[i1 + 9 + ((length)*i)]*256+bytes[i1 + 10 + ((length)*i)])
                    countDown=parseInt(countDown,4)
    ...
    while(flag===0) {
        ...
        if ((csd[3] === "1") && (csd[4] === "0")) {
            ...
            if (csd[1] === "1") {
                qual = "exceed"
                if (i2===0){
                    countUp=decimalToBitString(bytes[i1 + 9 + ((length)*i)])
                    countUp = parseInt(countUp, 2)
                } else {
                    countUp=decimalToBitString(bytes[i1 + 10 + ((length)*i)])
                    countUp = parseInt(countUp, 2)
                }
            } else {
                qual = "fall"
                if(i2===0){
                    countDown =decimalToBitString(bytes[i1 + 9 + ((length)*i)])
                    countDown = parseInt(countDown, 2)
                } else {
                    countDown =decimalToBitString(bytes[i1 + 10 + ((length)*i)])
                    countDown = parseInt(countDown, 2)
                }
            }
            let temp = ""
